...
/** True if transcript is "microphone off" or "hey robot". */
function isMicCommandPhrase(transcript: string): boolean {
  const t = normalizeTranscript(transcript);
  return t.includes('microphone off') || t.includes('hey robot');
}

export interface RealtimeSessionCallbacks {
  onConnectionChange?: (status: SessionStatus) => void;
  onAgentHandoff?: (agentName: string) => void;
  /** When user input is detected as "microphone off" or "hey robot" (e.g. to play a short ack). */
