...
|--------|--------|----------------|
| **Iframe load** | `handleIframeLoad` | After 1s, sends `enabled: true`. **Default at startup: mic ON** (so AI can hear and respond to chord display, queue add, etc.). |
...
| Trigger | Where | What happens |
|--------|--------|----------------|
| **User says “microphone on”** or clicks Enable | `handleMicCommand(true)` | Sends `enabled: true`, sets active mode to `backend_mic` (unless in metronome/backing_track). |
...
- **When the user says “Play”** (resume Spotify): the frontend turns the backend mic **off** again and clears `backend_mic` mode so the mic stays off until the user says “microphone on” (while paused) or “Stop”.
...
- **When the user says “Pause”** (Spotify): playback pauses; the backend mic is **not** turned on automatically. The user can say **“microphone on”** to turn the backend mic on and talk to the AI. When they say **“Play”** again, the mic is turned off again.
...
- **While Spotify is paused**, the user can say “microphone on” to turn the mic on, then “Play” to resume and turn the mic off again.
...
- **`null`**: No special mode; mic state is whatever was last set (e.g. on after iframe load, or off after “microphone on”).
...
|--------------------------|---------------------|-------------------------|
| Iframe load              | —                   | **ON** (default)        |
| “microphone on” / Enable | any                 | **ON**                  |
...
| “microphone on” (e.g. while Spotify paused) | any | **ON**  |
| Chord display / guitar tab | unchanged        | **Unchanged**           |

So: **Spotify turns the backend mic off when playing or resuming. When the user says “Stop”, the mic is turned back on. While paused, the user can say “microphone on” to talk to the AI, then “Play” to resume and turn the mic off again.**
