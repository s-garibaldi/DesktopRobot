...
export default function SpotifyPanel({ backendUrl, onPlaybackStateChange, onStop, useBackendForPlayback = false, sendToBackendIframe }: SpotifyPanelProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('keydown', primeActivation, true);
    return () => {
      window.removeEventListener('pointerdown', primeActivation, true);
      window.removeEventListener('keydown', primeActivation, true);
    };
  ...
  useEffect(() => {
    const hasCode = typeof window !== 'undefined' && Boolean(new URLSearchParams(window.location.search).get('code'));
